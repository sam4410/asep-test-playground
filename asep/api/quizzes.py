...
from asep.db.models import Quiz, User, Question, QuizSubmission
...
@router.post("/quizzes/submit", response_model=dict)
def submit_quiz(submission: QuizSubmissionCreate, db: Session = Depends(get_db)):
    ...
    return {"detail": "Quiz submitted successfully", "score": score}

@router.get("/quizzes/{quiz_id}/results", response_model=dict)
def get_quiz_results(quiz_id: UUID, db: Session = Depends(get_db)):
    results = db.query(QuizSubmission).filter(QuizSubmission.quiz_id == quiz_id).all()
    return {"results": [result.answers for result in results]}
from fastapi import APIRouter, HTTPException, Depends
from sqlalchemy.orm import Session
...
def submit_quiz(submission: QuizSubmissionCreate, db: Session = Depends(get_db)):
    ...
    return {"submission": new_submission, "score": score}
